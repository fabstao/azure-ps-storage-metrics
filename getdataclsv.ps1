...
Foreach ($i in $ResourceId) 
{
	...
	Foreach ($j in $roles) {
		...
		# Get Staging CPU
		Get-AzStgMetrics -ResId $i/slots/staging/roles/$j -ResName $ResourceName -StTime $fecha -TGrain 01:00:00 -MetricN "Percentage CPU" -Environment "staging" -Rrid cpustg$j.csv

		# Get Network Ingress
		Get-AzStgMetrics -ResId $i/slots/staging/roles/$j -ResName $ResourceName -StTime $fecha -TGrain 01:00:00 -MetricN "Network In" -Environment "staging" -Rrid instg$j.csv
		
		# Get Network Egress/Out
		Get-AzStgMetrics -ResId $i/slots/staging/roles/$j -ResName $ResourceName -StTime $fecha -TGrain 01:00:00 -MetricN "Network Out" -Environment "staging" -Rrid outstg$j.csv

		# Get Production CPU
		Get-AzStgMetrics -ResId $i/slots/production/roles/$j -ResName $ResourceName -StTime $fecha -TGrain 01:00:00 -MetricN "Percentage CPU" -Environment "production" -Rrid cpuprod$j.csv

		# Get Network Ingress
		Get-AzStgMetrics -ResId $i/slots/production/roles/$j -ResName $ResourceName -StTime $fecha -TGrain 01:00:00 -MetricN "Network In" -Environment "production" -Rrid inprod$j.csv
		
		# Get Network Egress/Out
		Get-AzStgMetrics -ResId $i/slots/production/roles/$j -ResName $ResourceName -StTime $fecha -TGrain 01:00:00 -MetricN "Network Out" -Environment "production" -Rrid outprod$j.csv
